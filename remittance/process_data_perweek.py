...
df_ognf4 = pd.read_excel(filename, sheet_name='OGNF4Details', header=0, usecols="B:AE")
df_micf4 = pd.read_excel(filename, sheet_name='MICF4Details', header=0, usecols="B:AE")
...
conn = engine.connect()
query = text(f"DELETE FROM f4detail WHERE lcode = ''")
conn.execute(query)
conn.close()
